...
//        scnView.addGestureRecognizer(longRecongnizer)
    ...
    func runStopWorkItem() {
        ...
        DispatchQueue.global().asyncAfter(deadline: .now()+0.4) {
            if self.hero.state != .longJump {
                print("runStopWorkItem stay")
...
extension TouchControllerOld {
    ...
    @objc func handlePanGesture() {
        ...
        case .changed:
            ...
            if hero.state != .highJump &&  hero.state != .longJump {
                print("handlePanGesture change")
                hero.state = .run
            }
        case .ended, .failed:
            runStopWorkItem()
    ...
    func move(){
        ...
        //    let velocity = getVelocity(startPoint: startAnchorPoint, nextPoint: currentAnchorPoint, newAxisSector: &newAxisSector)
...
extension TouchControllerOld {
    ...
    func longJump() {
        guard hero.state != .longJump else { return }
        hero.state = .longJump
        let duration = jumpDurationKf
        
        // Bounce:
        let bounceUpAction = SCNAction.moveBy(x: 0, y: calcJumpHigh(), z: 0, duration: duration * 0.5)
        let bounceDownAction = SCNAction.moveBy(x: 0, y: -1*calcJumpHigh(), z: 0, duration: duration * 0.3)
